...
export default function Categories() {
  ...
  return (
    ...
    <ScrollView horizontal showsHorizontalScrollIndicator={false} className="mt-3">
      <TouchableOpacity className={`mr-3 rounded-full ${isDarkMode ? 'bg-primary-light/60' : 'bg-primary-dark/70'} px-6 py-2`}>
        <Text className={`${isDarkMode ? 'text-primary-dark' : 'text-primary-light'}`}>Tous</Text>
      </TouchableOpacity>
      <TouchableOpacity className={`mr-3 rounded-full ${isDarkMode ? 'border border-2 border-white/30' : 'border border-2 border-primary-dark/30'} px-6 py-2`}>
      ...
      <TouchableOpacity className={`mr-3 rounded-full ${isDarkMode ? 'border border-2 border-white/30' : 'border border-2 border-primary-dark/30'} px-6 py-2`}>
        <Text className={`${isDarkMode ? 'text-white' : 'text-primary-dark'}`}>Échange linguistique</Text>
      </TouchableOpacity>
      <TouchableOpacity className={`mr-3 rounded-full ${isDarkMode ? 'border border-2 border-white/30' : 'border border-2 border-primary-dark/30'} px-6 py-2`}>
