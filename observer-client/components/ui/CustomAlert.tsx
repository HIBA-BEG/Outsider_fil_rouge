    ...
    <Modal visible={visible} transparent animationType="fade">
      <View className="flex-1 items-center justify-center bg-black/50">
        <View
          ...
          <Text
            className={`mb-6 text-center ${isDarkMode ? 'text-white/80' : 'text-primary-dark/80'}`}>
            {message}
          </Text>
          <View className="flex-row justify-end gap-4">
            {buttons.map((button, index) => (
              <TouchableOpacity key={index} onPress={button.onPress} className="px-4 py-2">
                <Text className={getButtonStyle(button.style)}>{button.text}</Text>
              </TouchableOpacity>
            ))}
          </View>
        </View>
